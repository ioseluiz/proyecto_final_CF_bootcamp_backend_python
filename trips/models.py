...
class Bus(models.Model):
    license_plate = models.CharField(
        unique=True, max_length=6, validators=[MinLengthValidator(4)]
    )
    total_seats = models.IntegerField(choices=BUS_TYPES)
    brand = models.CharField(max_length=30)
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="owner_bus"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    modified_at = models.DateTimeField(auto_now=True)
...
class Route(models.Model):
    origin = models.ForeignKey(
        Terminal, on_delete=models.CASCADE, related_name="terminal_origin"
    )
    destination = models.ForeignKey(
        Terminal, on_delete=models.CASCADE, related_name="terminal_destination"
    )
...
class Trip(models.Model):
    ...
    route = models.ForeignKey(
        Route, on_delete=models.CASCADE, related_name="trip_route"
    )
